...
phi = eye(2);
Gamma = samp_t*eye(2);
eps = 0.1;  %epsilon in the inequality below (28)
...
%         for k =2 : n_obs
%             
%             ilhs = norm(state((k-2),vectom(Kxt(:,I{j}(i)),n), phi, Gamma, lxt{j}(:,i), Xobs(:,1)) - Xobs(:,k)); %factor of 10 because the difference in the sample times
%            
%             if ilhs > eps
%                  dec = dec*0;
%             end
